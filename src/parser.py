import json
import logging
import re
import unicodedata
from pathlib import Path

import pandas as pd
from opencc import OpenCC

# https://phabricator.wikimedia.org/source/mediawiki/browse/master/includes/languages/data/ZhConversion.php
# 异体字处理
ZH_CHARS = {
    "「": "“",
    "｢": "“",
    "」": "”",
    "｣": "”",
    "『": "‘",
    "』": "’",
    "丣": "卯",
    "乗": "乘",
    "乹": "干",
    "亁": "干",
    "亝": "斋",
    "亱": "夜",
    "亷": "廉",
    "亾": "亡",
    "俻": "备",
    "倂": "并",
    "倐": "倏",
    "倣": "仿",
    "倸": "睬",
    "傚": "效",
    "僊": "仙",
    "働": "动",
    "儌": "侥",
    "兎": "兔",
    "兠": "兜",
    "冄": "冉",
    "冐": "冒",
    "冺": "泯",
    "凢": "凡",
    "凴": "凭",
    "刦": "劫",
    "刧": "劫",
    "刼": "劫",
    "剉": "锉",
    "剏": "创",
    "剙": "创",
    "剳": "札",
    "剹": "戮",
    "劄": "札",
    "劒": "剑",
    "効": "效",
    "勅": "敕",
    "勌": "倦",
    "勑": "敕",
    "勦": "剿",
    "匟": "炕",
    "匲": "奁",
    "匳": "奁",
    "厀": "膝",
    "厰": "厂",
    "吚": "咿",
    "吿": "告",
    "呌": "叫",
    "呪": "咒",
    "咊": "和",
    "哶": "咩",
    "唕": "唣",
    "唘": "启",
    "唚": "吣",
    "啎": "忤",
    "啑": "喋",
    "啗": "啖",
    "啣": "衔",
    "嗁": "啼",
    "嘑": "呼",
    "嘠": "嘎",
    "嘷": "嗥",
    "噁": "𫫇",
    "噉": "啖",
    "囓": "啮",
    "囙": "因",
    "圅": "函",
    "坿": "附",
    "垜": "垛",
    "塟": "葬",
    "塲": "场",
    "墖": "塔",
    "壻": "婿",
    "夘": "卯",
    "妬": "妒",
    "妳": "你",
    "姉": "姊",
    "姙": "妊",
    "姪": "侄",
    "姸": "妍",
    "婣": "姻",
    "婬": "淫",
    "媍": "妇",
    "媿": "愧",
    "嫰": "嫩",
    "嬝": "袅",
    "嬭": "奶",
    "嬾": "懒",
    "孼": "孽",
    "宂": "冗",
    "寃": "冤",
    "寑": "寝",
    "寳": "宝",
    "尅": "克",
    "尒": "尔",
    "尙": "尚",
    "尟": "鲜",
    "尠": "鲜",
    "屛": "屏",
    "屭": "屃",
    "岅": "坂",
    "峝": "峒",
    "嶃": "崭",
    "巗": "岩",
    "巠": "𢀖",
    "巵": "卮",
    "帀": "匝",
    "帋": "纸",
    "帬": "裙",
    "幇": "帮",
    "幑": "徽",
    "幙": "幕",
    "幚": "帮",
    "幷": "并",
    "庻": "庶",
    "庽": "寓",
    "廐": "厩",
    "廵": "巡",
    "廹": "迫",
    "廻": "回",
    "彞": "彝",
    "徃": "往",
    "徧": "遍",
    "怱": "匆",
    "怳": "恍",
    "恠": "怪",
    "恡": "吝",
    "悤": "匆",
    "惏": "婪",
    "惥": "恿",
    "惷": "蠢",
    "愽": "博",
    "慙": "惭",
    "慴": "慑",
    "慽": "戚",
    "憇": "憩",
    "憍": "㤭",
    "懧": "㤖",
    "戞": "戛",
    "戯": "戏",
    "戹": "厄",
    "扞": "捍",
    "抝": "拗",
    "拚": "拼",
    "挵": "弄",
    "捄": "救",
    "掽": "碰",
    "揑": "捏",
    "揫": "揪",
    "揷": "插",
    "揹": "背",
    "搆": "构",
    "搇": "揿",
    "搉": "榷",
    "搤": "扼",
    "搥": "捶",
    "搨": "拓",
    "搯": "掏",
    "搾": "榨",
    "摃": "扛",
    "撡": "操",
    "撦": "扯",
    "擕": "携",
    "擡": "抬",
    "擣": "𢭏",
    "擥": "㧛",
    "擧": "举",
    "攩": "挡",
    "攷": "考",
    "敂": "叩",
    "敍": "叙",
    "敺": "驱",
    "旤": "祸",
    "旹": "时",
    "旾": "春",
    "昬": "昏",
    "暱": "昵",
    "曁": "暨",
    "曡": "叠",
    "朞": "期",
    "朢": "望",
    "朶": "朵",
    "杴": "锨",
    "枱": "台",
    "栁": "柳",
    "栢": "柏",
    "栰": "筏",
    "桒": "桑",
    "桮": "杯",
    "桺": "柳",
    "椗": "碇",
    "椶": "棕",
    "椷": "缄",
    "椾": "笺",
    "楥": "楦",
    "槕": "桌",
    "槪": "概",
    "橜": "橛",
    "櫈": "凳",
    "櫺": "棂",
    "欝": "郁",
    "欵": "款",
    "歛": "敛",
    "歴": "历",
    "殀": "夭",
    "毧": "绒",
    "毬": "球",
    "氊": "毡",
    "氷": "冰",
    "汚": "污",
    "涖": "莅",
    "淛": "浙",
    "湼": "涅",
    "滛": "淫",
    "潄": "漱",
    "澁": "涩",
    "濇": "涩",
    "濶": "阔",
    "灋": "法",
    "灠": "漤",
    "烖": "灾",
    "煑": "煮",
    "煗": "暖",
    "熈": "熙",
    "燄": "焰",
    "爗": "烨",
    "牋": "笺",
    "牎": "窗",
    "牐": "闸",
    "牓": "榜",
    "牕": "窗",
    "牠": "它",
    "牴": "抵",
    "狥": "徇",
    "猂": "悍",
    "猨": "猿",
    "獘": "毙",
    "獧": "狷",
    "玅": "妙",
    "琖": "盏",
    "瑇": "玳",
    "瑯": "琅",
    "璢": "瑠",
    "甎": "砖",
    "甖": "罂",
    "甞": "尝",
    "畂": "亩",
    "畆": "亩",
    "畧": "略",
    "畮": "亩",
    "畱": "留",
    "疎": "疏",
    "疘": "肛",
    "疿": "痱",
    "痐": "蛔",
    "痲": "痳",
    "痺": "痹",
    "瘉": "愈",
    "瘖": "喑",
    "癄": "憔",
    "癅": "瘤",
    "癈": "废",
    "皐": "皋",
    "皜": "皓",
    "皷": "鼓",
    "盇": "盍",
    "盌": "碗",
    "眎": "视",
    "眡": "视",
    "睠": "眷",
    "睪": "睾",
    "瞇": "眯",
    "瞖": "翳",
    "瞭": "了",
    "矁": "瞅",
    "矙": "瞰",
    "矽": "硅",
    "砈": "砹",
    "砲": "炮",
    "硏": "研",
    "碪": "砧",
    "磟": "碌",
    "礆": "碱",
    "礮": "炮",
    "祘": "算",
    "秊": "年",
    "秌": "秋",
    "秖": "只",
    "稉": "粳",
    "稬": "糯",
    "稭": "秸",
    "稾": "稿",
    "穤": "糯",
    "穨": "颓",
    "穭": "稆",
    "穽": "阱",
    "窓": "窗",
    "窰": "窑",
    "窻": "窗",
    "竚": "伫",
    "竝": "并",
    "竢": "俟",
    "筞": "策",
    "筩": "筒",
    "筯": "箸",
    "箒": "帚",
    "箠": "棰",
    "篛": "箬",
    "簒": "篡",
    "簮": "簪",
    "簷": "檐",
    "籐": "藤",
    "粃": "秕",
    "粧": "妆",
    "紥": "扎",
    "絏": "绁",
    "緐": "繁",
    "緜": "绵",
    "緥": "褓",
    "繐": "穗",
    "繖": "伞",
    "繙": "翻",
    "繦": "襁",
    "罇": "樽",
    "罋": "瓮",
    "罸": "罚",
    "羗": "羌",
    "羢": "绒",
    "翄": "翅",
    "翶": "翱",
    "耡": "锄",
    "肎": "肯",
    "肐": "胳",
    "肧": "胚",
    "胷": "胸",
    "脃": "脆",
    "脇": "胁",
    "脗": "吻",
    "腁": "胼",
    "膓": "肠",
    "臈": "腊",
    "臋": "臀",
    "臕": "膘",
    "臙": "胭",
    "臝": "裸",
    "臯": "皋",
    "舖": "铺",
    "舩": "船",
    "艢": "樯",
    "艣": "橹",
    "艪": "橹",
    "芲": "花",
    "茘": "荔",
    "荳": "豆",
    "菫": "堇",
    "萲": "萱",
    "葠": "参",
    "蓡": "参",
    "蔕": "蒂",
    "蕚": "萼",
    "蕿": "萱",
    "藷": "薯",
    "藼": "萱",
    "蘋": "𬞟",
    "蘐": "萱",
    "蘓": "苏",
    "蘤": "花",
    "蚘": "蛔",
    "蛕": "蛔",
    "蜋": "螂",
    "蜖": "蛔",
    "蜨": "蝶",
    "蝯": "猿",
    "蝱": "虻",
    "螎": "融",
    "螡": "蚊",
    "蟁": "蚊",
    "蟇": "蟆",
    "蠏": "蟹",
    "蠒": "茧",
    "蠭": "蜂",
    "衂": "衄",
    "衇": "脉",
    "衞": "卫",
    "衹": "只",
    "衺": "邪",
    "袟": "帙",
    "袵": "衽",
    "裌": "袷",
    "裠": "裙",
    "褭": "袅",
    "襃": "褒",
    "襍": "杂",
    "覇": "霸",
    "覊": "羁",
    "覔": "觅",
    "覩": "睹",
    "觝": "抵",
    "詶": "酬",
    "誖": "悖",
    "謌": "歌",
    "譆": "嘻",
    "譌": "讹",
    "譔": "撰",
    "譼": "䛓",
    "讁": "谪",
    "讎": "仇",
    "讐": "雠",
    "豓": "艳",
    "貍": "狸",
    "貛": "獾",
    "賉": "恤",
    "賛": "赞",
    "賷": "赍",
    "贑": "赣",
    "趂": "趁",
    "跥": "跺",
    "踁": "胫",
    "踫": "碰",
    "蹏": "蹄",
    "蹔": "暂",
    "蹧": "糟",
    "蹵": "蹴",
    "躭": "耽",
    "躳": "躬",
    "躶": "裸",
    "輙": "辄",
    "輭": "软",
    "辠": "罪",
    "辢": "辣",
    "辤": "辞",
    "辳": "农",
    "迻": "移",
    "逈": "迥",
    "逥": "回",
    "逩": "奔",
    "逬": "迸",
    "遉": "侦",
    "遯": "遁",
    "遶": "绕",
    "酖": "鸩",
    "酧": "酬",
    "醆": "盏",
    "醕": "醇",
    "醯": "酰",
    "醻": "酬",
    "醼": "宴",
    "釬": "焊",
    "鉄": "铁",
    "鉅": "钜",
    "鉨": "鿭",
    "鉲": "锎",
    "銲": "焊",
    "鋂": "镅",
    "鍫": "锹",
    "鍳": "鉴",
    "鍾": "锺",
    "鎗": "枪",
    "鎻": "锁",
    "鏇": "镟",
    "鏚": "戚",
    "鑀": "锿",
    "鑈": "鿭",
    "鑚": "钻",
    "鑛": "矿",
    "鑤": "刨",
    "鑵": "罐",
    "钁": "䦆",
    "閙": "闹",
    "閤": "阁",
    "閧": "哄",
    "闚": "窥",
    "阨": "厄",
    "阬": "坑",
    "陗": "峭",
    "陻": "堙",
    "陿": "狭",
    "隂": "阴",
    "隄": "堤",
    "隖": "坞",
    "隣": "邻",
    "隷": "隶",
    "靦": "䩄",
    "靭": "韧",
    "靱": "韧",
    "鞀": "鼗",
    "鞵": "鞋",
    "鞾": "靴",
    "韈": "袜",
    "韤": "袜",
    "韮": "韭",
    "頟": "额",
    "頼": "赖",
    "顇": "悴",
    "顋": "腮",
    "顦": "憔",
    "飃": "飘",
    "飜": "翻",
    "飤": "饲",
    "餁": "饪",
    "餹": "糖",
    "餻": "糕",
    "餽": "馈",
    "饍": "膳",
    "饝": "馍",
    "駈": "驱",
    "騐": "验",
    "騣": "鬃",
    "驘": "骡",
    "骽": "腿",
    "骾": "鲠",
    "髈": "膀",
    "髥": "髯",
    "鬀": "剃",
    "鬉": "鬃",
    "鬦": "斗",
    "鬪": "斗",
    "鬭": "斗",
    "鬰": "郁",
    "鴈": "雁",
    "鵞": "鹅",
    "鵶": "鸦",
    "鷈": "䴘",
    "鷰": "燕",
    "鷿": "䴙",
    "鸎": "莺",
    "鹻": "碱",
    "麴": "麹",
    "鼃": "蛙",
    "鼇": "鳌",
    "鼈": "鳖",
    "鼕": "咚",
    "齩": "咬",
    "鿐": "䲤",
    "㒺": "罔",
    "㓂": "寇",
    "㕁": "却",
    "㕑": "厨",
    "㕘": "参",
    "㕥": "以",
    "㘭": "坳",
    "㝛": "宿",
    "㝠": "冥",
    "㠀": "岛",
    "㠯": "以",
    "㠶": "帆",
    "㡌": "帽",
    "㢘": "廉",
    "㤙": "恩",
    "㥦": "惬",
    "㨗": "捷",
    "㨪": "晃",
    "㨿": "据",
    "㩗": "携",
    "㩦": "携",
    "㪚": "散",
    "㪟": "敦",
    "㬉": "暖",
    "㬪": "叠",
    "㯭": "橹",
    "㱃": "饮",
    "㳒": "法",
    "㴱": "深",
    "㼝": "碗",
    "㽞": "留",
    "㿜": "瘪",
    "䈰": "筲",
    "䊀": "糊",
    "䖏": "处",
    "䗬": "蜂",
    "䘏": "恤",
    "䘑": "脉",
    "䘚": "卒",
    "䛐": "词",
    "䛡": "话",
    "䝔": "獾",
    "䠀": "蹚",
    "䠶": "射",
    "䥥": "镰",
    "䬃": "飒",
    "䭾": "驮",
    "䰟": "魂",
    "䳘": "鹅",
    "䶊": "衄",
    "𠕂": "再",
    "𠕅": "再",
    "𠞰": "剿",
    "𡚁": "弊",
    "𡨥": "寇",
    "𢣏": "㦈",
    "𣙜": "榷",
    "𣞁": "㮠",
    "𤋮": "熙",
    "𤨏": "琐",
    "𤱈": "亩",
    "𤷽": "㾡",
    "𤺥": "瘩",
    "𥵜": "䇚",
    "𦊱": "挂",
    "𦍑": "羌",
    "𦕈": "眇",
    "𦵏": "葬",
    "𧍕": "䖼",
    "𨥙": "𰽟",
    "𨦡": "𰽽",
    "𨽻": "隶",
    "𩓐": "脖",
    "𩗗": "飓",
    "𩗡": "𩙧",
    "𩷓": "鿵",
    "𩷕": "鿶",
    "𩷶": "𱇮",
    "𪄳": "鿺",
    "𪈼": "𪉓",
    "𪋿": "𪎍",
    "𮨻": "𬲱",
    "𰡓": "𰡉",
    "𰻞": "𰻝",
    "𱆥": "鿕",
    "𲇊": "𬭄",
}

# 两岸用语转换；繁简非一对一补充
# "公升": "升",
# "公尺": "米",
# "幫浦": "泵",
ZH_WORDS = {
    "·普丁": "·普京",
    "16進位": "16进制",
    "A型肝炎": "甲型肝炎",
    "A肝": "甲肝",
    "B型肝炎": "乙型肝炎",
    "B肝": "乙肝",
    "C型肝炎": "丙型肝炎",
    "C肝": "丙肝",
    "IP位址": "IP地址",
    "一份子": "一分子",
    "三十六著": "三十六着",
    "三極體": "三极管",
    "三藩市": "旧金山",
    "三蘭港": "达累斯萨拉姆",
    "上落客": "上下客",
    "下著": "下着",
    "不著": "不着",
    "主機板": "主板",
    "么九": "幺九",
    "么二三": "幺二三",
    "么兒": "幺儿",
    "么半": "幺半",
    "么叔": "幺叔",
    "么喝": "幺喝",
    "么女": "幺女",
    "么妹": "幺妹",
    "么姓": "幺姓",
    "么姨": "幺姨",
    "么娘": "幺娘",
    "么媽": "幺妈",
    "么孃": "幺娘",
    "么子": "幺子",
    "么弟": "幺弟",
    "么正": "幺正",
    "么氏": "幺氏",
    "么爸": "幺爸",
    "么爹": "幺爹",
    "么爺": "幺爷",
    "么篇": "幺篇",
    "么舅": "幺舅",
    "么蛾子": "幺蛾子",
    "么雞": "幺鸡",
    "么鳳": "幺凤",
    "乘著": "乘着",
    "乙份": "一份",
    "乙個": "一个",
    "乙隻": "一只",
    "乾一壇": "乾一坛",
    "乾一組": "乾一组",
    "乾儀": "乾仪",
    "乾剛": "乾刚",
    "乾劉": "乾刘",
    "乾務": "乾务",
    "乾啟": "乾启",
    "乾姊": "干姐",
    "乾寧": "乾宁",
    "乾岡": "乾冈",
    "乾崗": "乾岗",
    "乾暉": "乾晖",
    "乾東": "乾东",
    "乾棟": "乾栋",
    "乾構": "乾构",
    "乾樞": "乾枢",
    "乾為天": "乾为天",
    "乾為陽": "乾为阳",
    "乾竇": "乾窦",
    "乾篤": "乾笃",
    "乾紐": "乾纽",
    "乾絡": "乾络",
    "乾統": "乾统",
    "乾維": "乾维",
    "乾羅": "乾罗",
    "乾興": "乾兴",
    "乾華": "乾华",
    "乾蔭": "乾荫",
    "乾貞": "乾贞",
    "乾貴士": "乾贵士",
    "乾貺": "乾贶",
    "乾車": "乾车",
    "乾軸": "乾轴",
    "乾鈞": "乾钧",
    "乾錄": "乾录",
    "乾鑒": "乾鉴",
    "乾闥": "乾闼",
    "乾陽": "乾阳",
    "乾靈": "乾灵",
    "乾顧": "乾顾",
    "乾風": "乾风",
    "乾馬": "乾马",
    "乾鵠": "乾鹄",
    "乾鵲": "乾鹊",
    "乾龍": "乾龙",
    "二極體": "二极管",
    "二進位": "二进制",
    "亚细安": "东盟",
    "亞加力": "亚克力",
    "亞塞拜然": "阿塞拜疆",
    "亞斯文": "阿斯旺",
    "亞歷山卓": "亚历山大",
    "亞里斯多德": "亚里士多德",
    "交帳": "交账",
    "亮著": "亮着",
    "人工智慧": "人工智能",
    "什勒斯維希": "石勒苏益格",
    "介面": "界面",
    "仗著": "仗着",
    "付帳": "付账",
    "代表著": "代表着",
    "仲介": "中介",
    "份內": "分内",
    "份外": "分外",
    "份量": "分量",
    "伊利沙伯二世": "伊丽莎白二世",
    "伊利諾": "伊利诺伊",
    "伊斯坦堡": "伊斯坦布尔",
    "伊斯蘭瑪巴德": "伊斯兰堡",
    "伊東豊雄": "伊东丰雄",
    "伊波拉": "埃博拉",
    "伊莉莎白": "伊丽莎白",
    "伏地挺身": "俯卧撑",
    "伏著": "伏着",
    "伯明罕": "伯明翰",
    "伴著": "伴着",
    "伺服器": "服务器",
    "位元組": "字节",
    "低著": "低着",
    "住著": "住着",
    "作業系統": "操作系统",
    "來著": "来着",
    "侏儸紀": "侏罗纪",
    "促著": "促着",
    "保鑣": "保镖",
    "保障著": "保障着",
    "信著": "信着",
    "倒帳": "倒账",
    "候著": "候着",
    "候覆": "候复",
    "倚著": "倚着",
    "借著": "借着",
    "假帳": "假账",
    "做著": "做着",
    "健力士世界": "吉尼斯世界",
    "側著": "侧着",
    "偷著": "偷着",
    "傅利葉": "傅里叶",
    "備著": "备着",
    "傳著": "传着",
    "優先順序": "优先级",
    "光碟機": "光驱",
    "光著": "光着",
    "克卜勒": "开普勒",
    "克羅埃西亞": "克罗地亚",
    "兒寬": "兒宽",
    "入帳": "入账",
    "全球資訊網": "万维网",
    "八進位": "八进制",
    "公厘": "毫米",
    "公帳": "公账",
    "公釐": "毫米",
    "六么": "六幺",
    "六進位": "六进制",
    "共和联邦": "英联邦",
    "冒著": "冒着",
    "凱薩琳": "凯瑟琳",
    "分佈著": "分布着",
    "分布著": "分布着",
    "切爾諾貝爾": "切尔诺贝利",
    "列印": "打印",
    "列支敦斯登": "列支敦士登",
    "別著": "别着",
    "刮著": "刮着",
    "到帳": "到账",
    "制著": "制着",
    "刻著": "刻着",
    "前波莫瑞": "前波美拉尼亚",
    "劃著": "划着",
    "功能變數名稱": "域名",
    "加彭": "加蓬",
    "加泰隆尼亞": "加泰罗尼亚",
    "加薩走廊": "加沙地带",
    "努力著": "努力着",
    "動著": "动着",
    "包著": "包着",
    "北韓": "朝鲜",
    "匯流排": "总线",
    "區域網": "局域网",
    "區域網路": "局域网络",
    "十進位": "十进制",
    "千里達": "特立尼达",
    "千里達及托巴哥": "特立尼达和多巴哥",
    "卑詩省": "不列颠哥伦比亚省",
    "南北韓": "朝韩",
    "南斯拉夫聯邦共和國": "南斯拉夫联盟共和国",
    "南西·裴洛西": "南希·佩洛西",
    "南韓": "韩国",
    "博和託": "博和讬",
    "占士邦": "詹姆斯·邦德",
    "占著": "占着",
    "卡佩雅蒂": "卡普里亚蒂",
    "卡拉卡斯": "加拉加斯",
    "卡斯楚": "卡斯特罗",
    "卯足": "铆足",
    "印著": "印着",
    "印表機": "打印机",
    "厄利垂亚": "厄立特里亚",
    "厄利垂亞": "厄立特里亚",
    "厄瓜多": "厄瓜多尔",
    "原始碼": "源代码",
    "原子筆": "圆珠笔",
    "受著": "受着",
    "叫著": "叫着",
    "可攜式": "便携式",
    "台帳": "台账",
    "史丹佛大學": "斯坦福大学",
    "史丹福大學": "斯坦福大学",
    "史太林": "斯大林",
    "史特勞斯": "斯特劳斯",
    "史瓦帝尼": "斯威士兰",
    "史瓦濟蘭": "斯威士兰",
    "史達林": "斯大林",
    "史高比耶": "斯科普里",
    "司諾克": "斯诺克",
    "吃著": "吃着",
    "吉力馬札羅": "乞力马扎罗",
    "吉布地": "吉布提",
    "吉里巴斯": "基里巴斯",
    "吊著": "吊着",
    "名份": "名分",
    "吐瓦魯": "图瓦卢",
    "向著": "向着",
    "含著": "含着",
    "吳克羣": "吴克羣",
    "吹著": "吹着",
    "呆帳": "呆账",
    "呆著": "呆着",
    "味著": "味着",
    "咖哩": "咖喱",
    "咬著": "咬着",
    "哈瑪斯": "哈马斯",
    "哥德式": "哥特式",
    "哥斯大黎加": "哥斯达黎加",
    "哭著": "哭着",
    "唐納·川普": "唐纳德·特朗普",
    "唯讀": "只读",
    "唱著": "唱着",
    "問著": "问着",
    "喀拉蚩": "卡拉奇",
    "喊著": "喊着",
    "喜歡著": "喜欢着",
    "喝著": "喝着",
    "喬治·歐威爾": "乔治·奥威尔",
    "喬治亞字母": "格鲁吉亚字母",
    "喬治亞語": "格鲁吉亚语",
    "單眼相機": "单反相机",
    "單鏡反光機": "单反相机",
    "嗅著": "嗅着",
    "嘉芙蓮": "凯瑟琳",
    "嚴於信": "严於信",
    "嚷著": "嚷着",
    "回著": "回着",
    "因著": "因着",
    "困著": "困着",
    "固著": "固着",
    "圍著": "围着",
    "土魯斯": "图卢兹",
    "在等著": "在等着",
    "坎城": "戛纳",
    "坎培拉": "堪培拉",
    "坐著": "坐着",
    "坦尚尼亞": "坦桑尼亚",
    "埋著": "埋着",
    "執行長，": "首席执行官，",
    "執行長、": "首席执行官、",
    "執行長。": "首席执行官。",
    "堂姊": "堂姐",
    "堅貞著": "坚贞着",
    "報帳": "报账",
    "塑膠袋": "塑料袋",
    "塗著": "涂着",
    "塞席爾": "塞舌尔",
    "塞拉耶佛": "萨拉热窝",
    "塞普勒斯": "塞浦路斯",
    "塞爾維亞與蒙特內哥羅": "塞尔维亚和黑山",
    "塞維亞": "塞维利亚",
    "墨瀋": "墨渖",
    "壓克力": "亚克力",
    "壓著": "压着",
    "士多啤梨": "草莓",
    "士碌架": "斯诺克",
    "外掛程式": "插件",
    "外部連結": "外部链接",
    "多明尼加": "多米尼加",
    "夢著": "梦着",
    "大姊": "大姐",
    "大姊姊": "大姐姐",
    "大英國協": "英联邦",
    "天份": "天分",
    "天道為乾": "天道为乾",
    "太空梭": "航天飞机",
    "太空衣": "宇航服",
    "太空飛行員": "宇航员",
    "太陽電腦": "太阳微系统",
    "夾著": "夹着",
    "奈及利亞": "尼日利亚",
    "奈比多": "内比都",
    "奈洛比": "内罗毕",
    "奧勒岡": "俄勒冈",
    "奧區": "奧区",
    "奧塞提亞": "奧塞梯",
    "奧拉夫·蕭茲": "奥拉夫·朔尔茨",
    "如瀋": "如渖",
    "姊夫": "姐夫",
    "姊姊": "姐姐",
    "姊弟": "姐弟",
    "姓么": "姓幺",
    "威斯伐倫": "威斯特法伦",
    "威爾斯": "威尔士",
    "威騰電子": "西部数据",
    "子餘": "子馀",
    "字型大小": "字号",
    "存在著": "存在着",
    "存著": "存着",
    "孟德爾遜": "门德尔松",
    "孟德爾頌": "门德尔松",
    "孫乾": "孙乾",
    "學姊": "学姐",
    "學著": "学着",
    "守著": "守着",
    "安哈特": "安哈尔特",
    "安地卡": "安提瓜",
    "安地卡及巴布達": "安提瓜和巴布达",
    "宏都拉斯": "洪都拉斯",
    "定著": "定着",
    "宣告著": "宣告着",
    "宿霧": "宿务",
    "密执安": "密歇根",
    "密西根": "密歇根",
    "富比士": "福布斯",
    "寫著": "写着",
    "寮人民民主共和國": "老挝人民民主共和国",
    "寮保護國": "老挝保护国",
    "寮國": "老挝",
    "寮語": "老挝语",
    "將軍抽俥": "将军抽俥",
    "將軍抽車": "将军抽車",
    "尊敬著": "尊敬着",
    "尋著": "寻着",
    "對帳": "对账",
    "對著": "对着",
    "導向飛彈": "导弹",
    "尖峰時段": "高峰时段",
    "尖峰時間": "高峰时间",
    "尚吉巴": "桑给巴尔",
    "尚比亞": "赞比亚",
    "就等著": "就等着",
    "尼日共和國": "尼日尔共和国",
    "尾班車": "末班车",
    "展著": "展着",
    "山崎闇齋": "山崎闇斋",
    "岳託": "岳讬",
    "峇厘": "巴厘",
    "峇里": "巴厘",
    "工具機": "机床",
    "巴塞隆拿": "巴塞罗那",
    "巴塞隆納": "巴塞罗那",
    "巴斯拉": "巴士拉",
    "巴貝多": "巴巴多斯",
    "布吉納法索": "布基纳法索",
    "布拉提斯拉瓦": "布拉迪斯拉发",
    "布蘭登堡": "勃兰登堡",
    "布隆泉": "布隆方丹",
    "希冀著": "希冀着",
    "希拉莉": "希拉里",
    "希拉蕊": "希拉里",
    "希特拉": "希特勒",
    "帕委會": "残奥委会",
    "帕拉林匹克": "残疾人奥林匹克",
    "帕運": "残奥",
    "帕邁拉環礁": "巴尔米拉环礁",
    "帛琉": "帕劳",
    "席哈克": "希拉克",
    "帳上": "账上",
    "帳冊": "账册",
    "帳務": "账务",
    "帳單": "账单",
    "帳外": "账外",
    "帳戶": "账户",
    "帳房": "账房",
    "帳本": "账本",
    "帳款": "账款",
    "帳目": "账目",
    "帳簿": "账簿",
    "帳號": "账号",
    "帳面": "账面",
    "帶著": "带着",
    "幫著": "帮着",
    "幹著": "干着",
    "幹著稱": "幹著称",
    "幾內亞比索": "几内亚比绍",
    "庇里牛斯": "比利牛斯",
    "座標": "坐标",
    "庫德人": "库尔德人",
    "庫德斯坦": "库尔德斯坦",
    "庫德族": "库尔德族",
    "庫德語": "库尔德语",
    "康乃狄克": "康涅狄格",
    "建帳": "建账",
    "強斯頓環礁": "约翰斯顿岛",
    "形上學": "形而上学",
    "待著": "待着",
    "待覆": "待复",
    "得著": "得着",
    "御姊": "御姐",
    "循著": "循着",
    "徵狀": "症状",
    "德勒斯登": "德累斯顿",
    "德希達": "德里达",
    "德蕾莎·梅伊": "特蕾莎·梅",
    "心著": "心着",
    "忌廉": "奶油",
    "忍著": "忍着",
    "忙著": "忙着",
    "忠貞著": "忠贞着",
    "快閃記憶體": "闪存",
    "急著": "急着",
    "恐攻": "恐袭",
    "恩加美納": "恩贾梅纳",
    "恰如其份": "恰如其分",
    "悠著": "悠着",
    "悶著": "闷着",
    "情境喜劇": "情景喜剧",
    "想著": "想着",
    "愛滋": "艾滋",
    "愛著": "爱着",
    "慇勤": "殷勤",
    "慇懃": "殷勤",
    "慘澹": "惨淡",
    "慣著": "惯着",
    "慫慂": "怂恿",
    "慶餘": "庆馀",
    "憑著": "凭着",
    "應用程式": "应用程序",
    "應著": "应着",
    "懷著": "怀着",
    "戀著": "恋着",
    "戈巴卓夫": "戈尔巴乔夫",
    "戈巴契夫": "戈尔巴乔夫",
    "成份": "成分",
    "戰著": "战着",
    "戴卓爾": "撒切尔",
    "戴著": "戴着",
    "扎著": "扎着",
    "打著": "打着",
    "托巴哥": "多巴哥",
    "扛著": "扛着",
    "批覆": "批复",
    "抓著": "抓着",
    "投機份子": "投机分子",
    "披索": "比索",
    "披著": "披着",
    "披頭四": "披头士",
    "抬著": "抬着",
    "抱著": "抱着",
    "拉著": "拉着",
    "拎著": "拎着",
    "拖著": "拖着",
    "拼著": "拼着",
    "拾瀋": "拾渖",
    "拿著": "拿着",
    "持著": "持着",
    "指著": "指着",
    "挑著": "挑着",
    "挨著": "挨着",
    "捆著": "捆着",
    "掃瞄": "扫描",
    "掃瞄器": "扫描仪",
    "掌上壓": "俯卧撑",
    "掖著": "掖着",
    "掙著": "挣着",
    "掛著": "挂着",
    "接著": "接着",
    "控制項": "控件",
    "揉著": "揉着",
    "提比里西": "第比利斯",
    "提著": "提着",
    "握著": "握着",
    "揮著": "挥着",
    "搜尋引擎": "搜索引擎",
    "搶著": "抢着",
    "摟著": "搂着",
    "摩斯比港": "莫尔斯比港",
    "摸著": "摸着",
    "撑著": "撑着",
    "撒拉威阿拉伯": "阿拉伯撒哈拉",
    "撞球": "台球",
    "播著": "播着",
    "撼著": "撼着",
    "擊沈": "击沉",
    "擊著": "击着",
    "擋著": "挡着",
    "擔著": "担着",
    "據著": "据着",
    "擺著": "摆着",
    "攜帶型": "便携式",
    "收帳": "收账",
    "收錄著": "收录着",
    "放帳": "放账",
    "放著": "放着",
    "敞著": "敞着",
    "散佈著": "散布着",
    "散布著": "散布着",
    "敲著": "敲着",
    "數位化": "数字化",
    "數位技術": "数字技术",
    "數位照相機": "数码照相机",
    "數位相機": "数码相机",
    "數位訊號": "数字信号",
    "數位電視": "数字电视",
    "數位音樂": "数字音乐",
    "數據機": "调制解调器",
    "數碼訊號": "数字信号",
    "數碼電視": "数字电视",
    "數著": "数着",
    "文翠珊": "特蕾莎·梅",
    "斥著": "斥着",
    "斯堪地那維亞": "斯堪的纳维亚",
    "斯洛維尼亞": "斯洛文尼亚",
    "於之瑩": "於之莹",
    "於夫羅": "於夫罗",
    "於志賀": "於志贺",
    "於賢德": "於贤德",
    "昂山素姬": "昂山素季",
    "昂著": "昂着",
    "昇陽電腦": "太阳微系统",
    "明白帳": "明白账",
    "易經·乾": "易经·乾",
    "易經乾": "易经乾",
    "映著": "映着",
    "晃著": "晃着",
    "普利托利亞": "比勒陀利亚",
    "普利茲": "普利策",
    "普立茲獎": "普利策奖",
    "晶元": "芯片",
    "晶片": "芯片",
    "智慧卡": "智能卡",
    "智慧型": "智能",
    "智慧手機": "智能手机",
    "智慧機器人": "智能机器人",
    "智慧財產權": "知识产权",
    "智財權": "知识产权",
    "暗著": "暗着",
    "曠若發矇": "旷若发矇",
    "曾運乾": "曾运乾",
    "有著": "有着",
    "望著": "望着",
    "朝著": "朝着",
    "本份": "本分",
    "本帳": "本账",
    "本本份份": "本本分分",
    "本著": "本着",
    "李乾順": "李乾顺",
    "李奧納多": "列奥那多",
    "杜塞道夫": "杜塞尔多夫",
    "杜拜": "迪拜",
    "束著": "束着",
    "東加王國": "汤加王国",
    "東協": "东盟",
    "東南亞國家協會": "东南亚国家联盟",
    "板著臉": "板着脸",
    "枕著": "枕着",
    "柏林圍牆": "柏林墙",
    "查帳": "查账",
    "柯德莉·夏萍": "奥黛丽·赫本",
    "柯林頓": "克林顿",
    "柳詒徵": "柳诒徵",
    "柴契爾": "撒切尔",
    "格仔": "格子",
    "格林美獎": "格莱美奖",
    "格瑞那丁": "格林纳丁斯",
    "格瑞那達": "格林纳达",
    "桌上型電腦": "台式电脑",
    "梅赫西迪": "梅赛德斯",
    "梳著": "梳着",
    "梵谷": "梵高",
    "森巴舞": "桑巴舞",
    "槍枝": "枪支",
    "樂著": "乐着",
    "標志著": "标志着",
    "標誌著": "标志着",
    "模里西斯": "毛里求斯",
    "樸茨茅夫": "朴茨茅斯",
    "機械人": "机器人",
    "機率": "几率",
    "欠帳": "欠账",
    "歐巴馬": "奥巴马",
    "正等著": "正等着",
    "正體中文": "繁体中文",
    "死帳": "死账",
    "殘瀋": "残渖",
    "殺著": "杀着",
    "毛里裘斯": "毛里求斯",
    "氧份": "氧分",
    "水份": "水分",
    "水氣": "水汽",
    "求著": "求着",
    "汕埠": "圣佩德罗苏拉",
    "汶萊": "文莱",
    "沈澱": "沉淀",
    "沈著": "沉着",
    "沈重": "沉重",
    "沉著": "沉着",
    "沖帳": "冲账",
    "沖著": "冲着",
    "沙地阿拉伯": "沙特阿拉伯",
    "沙烏地": "沙特",
    "沿著": "沿着",
    "波士尼亞": "波斯尼亚",
    "波士尼亞與赫塞哥維納": "波斯尼亚和黑塞哥维那",
    "波札那": "博茨瓦纳",
    "波索納洛": "博索纳罗",
    "波里活": "宝莱坞",
    "洗鍊": "洗练",
    "活著": "活着",
    "派屈克": "帕特里克",
    "流動互聯網": "移动互联网",
    "流動作業系統": "移动操作系统",
    "流動應用程式": "移动应用程序",
    "流動支付": "移动支付",
    "流動網絡": "移动网络",
    "流動裝置": "移动设备",
    "流動電話": "移动电话",
    "流水帳": "流水账",
    "流著": "流着",
    "流露著": "流露着",
    "浮著": "浮着",
    "海洛英": "海洛因",
    "海浬": "海里",
    "涼著": "凉着",
    "混帳": "混账",
    "混著": "混着",
    "清帳": "清账",
    "渴著": "渴着",
    "湊合著": "凑合着",
    "溢著": "溢着",
    "溫荷克": "温得和克",
    "滑鼠": "鼠标",
    "滿著": "满着",
    "演化論": "进化论",
    "演算法": "算法",
    "演著": "演着",
    "漫著": "漫着",
    "潤著": "润着",
    "澈底": "彻底",
    "澤倫斯基": "泽连斯基",
    "瀋液": "渖液",
    "為著": "为着",
    "烏戈·查維茲": "乌戈·查韦斯",
    "烏龍麵": "乌冬面",
    "煞車": "刹车",
    "照著": "照着",
    "燒著": "烧着",
    "營運長，": "首席运营官，",
    "營運長、": "首席运营官、",
    "營運長。": "首席运营官。",
    "爛帳": "烂账",
    "爭著": "争着",
    "片語": "词组",
    "牽著": "牵着",
    "狂牛症": "疯牛病",
    "狗隻": "犬只",
    "猜著": "猜着",
    "猶豫著": "犹豫着",
    "獅子山共和國": "塞拉利昂共和国",
    "獨立國協": "独联体",
    "獨立國家國協": "独立国家联合体",
    "獲著": "获着",
    "王餘魚": "王馀鱼",
    "玩著": "玩着",
    "玻利維亞多民族國": "多民族玻利维亚国",
    "玻里尼西亞": "波利尼西亚",
    "班傑明·內塔尼亞胡": "本雅明·内塔尼亚胡",
    "班傑明·納坦雅胡": "本雅明·内塔尼亚胡",
    "班傑明": "本杰明",
    "瑪丹娜": "麦当娜",
    "瓜地馬拉": "危地马拉",
    "瓦倫西亞": "巴伦西亚",
    "甘比亞": "冈比亚",
    "甘迺迪": "肯尼迪",
    "甚麼": "什么",
    "甚麽": "什么",
    "甜著": "甜着",
    "用著": "用着",
    "男性為乾": "男性为乾",
    "男為乾": "男为乾",
    "留尼旺": "留尼汪",
    "留著": "留着",
    "畢卡索": "毕加索",
    "畫著": "画着",
    "當勞·特朗普": "唐纳德·特朗普",
    "當奴·特朗普": "唐纳德·特朗普",
    "當著": "当着",
    "畿內亞": "几内亚",
    "疊代": "迭代",
    "疑著": "疑着",
    "瘋牛症": "疯牛病",
    "發著": "发着",
    "白朗寧": "勃朗宁",
    "白瀋": "白渖",
    "百慕達": "百慕大",
    "皮雅斯·布士南": "皮尔斯·布鲁斯南",
    "皺著": "皱着",
    "盛著": "盛着",
    "盧安達": "卢旺达",
    "盯著": "盯着",
    "盼覆": "盼复",
    "看著": "看着",
    "真人騷": "真人秀",
    "睡著": "睡着",
    "瞞著": "瞒着",
    "瞧著": "瞧着",
    "瞪著": "瞪着",
    "矇矓": "矇眬",
    "矛盾著": "矛盾着",
    "知識份子": "知识分子",
    "矽片": "硅片",
    "矽谷": "硅谷",
    "硬碟": "硬盘",
    "硬著": "硬着",
    "硬體": "硬件",
    "碧咸": "贝克汉姆",
    "碩託": "硕讬",
    "磁碟": "磁盘",
    "磁軌": "磁道",
    "祇是": "只是",
    "祇有": "只有",
    "祇要": "只要",
    "祖雲達斯": "尤文图斯",
    "福著": "福着",
    "福馬林": "福尔马林",
    "私帳": "私账",
    "私隱": "隐私",
    "程式控制": "程控",
    "程式設計師": "程序员",
    "穀陽": "穀阳",
    "積架": "捷豹",
    "積極份子": "积极分子",
    "穫著": "获着",
    "空中巴士": "空中客车",
    "空氣品質": "空气质量",
    "空氣質素": "空气质量",
    "空著": "空着",
    "穿梭機": "航天飞机",
    "穿著": "穿着",
    "穿著者": "穿着者",
    "突尼西亞": "突尼斯",
    "立著": "立着",
    "站著": "站着",
    "笑著": "笑着",
    "笨豬跳": "蹦极跳",
    "筆帳": "笔账",
    "等著被": "等着被",
    "策畫": "策划",
    "算帳": "算账",
    "管帳": "管账",
    "管著": "管着",
    "簽帳": "签账",
    "簽著": "签着",
    "米瀋": "米渖",
    "米高·奧雲": "迈克尔·欧文",
    "糊塗帳": "糊涂账",
    "糖份": "糖分",
    "紀錄著": "纪录着",
    "約翰尼斯堡": "约翰内斯堡",
    "紐奧良": "新奥尔良",
    "紐幾內亞": "新几内亚",
    "紐澤西": "新泽西",
    "紐西蘭": "新西兰",
    "索忍尼辛": "索尔仁尼琴",
    "索羅門群島": "所罗门群岛",
    "索贊尼辛": "索尔仁尼琴",
    "索馬利亞": "索马里",
    "索馬利蘭": "索马里兰",
    "紮著": "扎着",
    "組份": "组分",
    "結他": "吉他",
    "結帳": "结账",
    "絪縕": "絪缊",
    "綁著": "绑着",
    "經常帳": "经常账",
    "經濟帳": "经济账",
    "維吉尼亞": "弗吉尼亚",
    "維德角": "佛得角",
    "維根斯坦": "维特根斯坦",
    "維珍尼亞": "弗吉尼亚",
    "網域名稱": "域名",
    "網站連結": "网站链接",
    "網路": "网络",
    "網際網絡": "互联网",
    "網際網路": "互联网",
    "網頁連結": "网页链接",
    "線國安": "缐国安",
    "線姓": "缐姓",
    "緣份": "缘分",
    "縱著": "纵着",
    "總帳": "总账",
    "繃著": "绷着",
    "織著": "织着",
    "繞著": "绕着",
    "繪著": "绘着",
    "繫著": "系着",
    "纏著": "缠着",
    "绑紧跳": "蹦极跳",
    "罩著": "罩着",
    "罵著": "骂着",
    "羅亞爾河": "卢瓦尔河",
    "羅浮宮": "卢浮宫",
    "羅蘋": "罗宾",
    "美式足球": "美式橄榄球",
    "美著": "美着",
    "義大利": "意大利",
    "義裔": "意裔",
    "翁山蘇姬": "昂山素季",
    "耀著": "耀着",
    "老么": "老幺",
    "考著": "考着",
    "耶加達": "雅加达",
    "聖佐治": "圣乔治",
    "聖克里斯多福及尼維斯": "圣基茨和尼维斯",
    "聖吉斯納域斯": "圣基茨和尼维斯",
    "聖地牙哥": "圣地亚哥",
    "聖多美普林西比": "圣多美和普林西比",
    "聖嬰現象": "厄尔尼诺现象",
    "聖文森及格瑞那丁": "圣文森特和格林纳丁斯",
    "聖文森國": "圣文森特和格林纳丁斯",
    "聖露西亞": "圣卢西亚",
    "聖馬利諾": "圣马力诺",
    "聞著": "闻着",
    "聶斯特河": "德涅斯特河",
    "職份": "职分",
    "聽著": "听着",
    "背著": "背着",
    "胺基酸": "氨基酸",
    "膠著": "胶着",
    "臥著": "卧着",
    "臨著": "临着",
    "與著": "与着",
    "舊帳": "旧账",
    "舒麥加": "舒马赫",
    "艾菲爾": "埃菲尔",
    "芮氏0": "里氏0",
    "芮氏1": "里氏1",
    "芮氏2": "里氏2",
    "芮氏3": "里氏3",
    "芮氏4": "里氏4",
    "芮氏5": "里氏5",
    "芮氏6": "里氏6",
    "芮氏7": "里氏7",
    "芮氏8": "里氏8",
    "芮氏9": "里氏9",
    "芮氏地震規模": "里氏地震规模",
    "芮氏規模": "里氏震级",
    "若且唯若": "当且仅当",
    "苦著": "苦着",
    "英吋": "英寸",
    "英呎": "英尺",
    "茅利塔尼亞": "毛里塔尼亚",
    "荷姆茲": "霍尔木兹",
    "荷尼阿拉": "霍尼亚拉",
    "荷里活": "好莱坞",
    "莫三比克": "莫桑比克",
    "莫士比港": "莫尔斯比港",
    "華倫西亞": "巴伦西亚",
    "華勒沙": "瓦文萨",
    "華里沙": "瓦文萨",
    "萌島": "马恩岛",
    "萬那杜": "瓦努阿图",
    "落著": "落着",
    "落車": "下车",
    "葉爾欽": "叶利钦",
    "葉里溫": "埃里温",
    "葉門": "也门",
    "著人": "着人",
    "著什": "着什",
    "著他": "着他",
    "著令": "着令",
    "著位": "着位",
    "著你": "着你",
    "著便": "着便",
    "著力": "着力",
    "著勁": "着劲",
    "著呢": "着呢",
    "著哩": "着哩",
    "著地": "着地",
    "著墨": "着墨",
    "著她": "着她",
    "著妳": "着你",
    "著它": "着它",
    "著實": "着实",
    "著己": "着己",
    "著床": "着床",
    "著式": "着式",
    "著心": "着心",
    "著志": "着志",
    "著忙": "着忙",
    "著急": "着急",
    "著惱": "着恼",
    "著想": "着想",
    "著意": "着意",
    "著慌": "着慌",
    "著我": "着我",
    "著手": "着手",
    "著數": "着数",
    "著棋": "着棋",
    "著業": "着业",
    "著極": "着极",
    "著氣": "着气",
    "著法": "着法",
    "著涼": "着凉",
    "著淺": "着浅",
    "著火": "着火",
    "著然": "着然",
    "著牠": "着它",
    "著甚": "着甚",
    "著甚麼": "着什么",
    "著生": "着生",
    "著疑": "着疑",
    "著白": "着白",
    "著相": "着相",
    "著眼": "着眼",
    "著祂": "着祂",
    "著筆": "着笔",
    "著絲": "着丝",
    "著綠": "着绿",
    "著緊": "着紧",
    "著緋": "着绯",
    "著肉": "着肉",
    "著腳": "着脚",
    "著艦": "着舰",
    "著色": "着色",
    "著花": "着花",
    "著莫": "着莫",
    "著落": "着落",
    "著著": "着着",
    "著處": "着处",
    "著衣": "着衣",
    "著裝": "着装",
    "著要": "着要",
    "著迷": "着迷",
    "著邊": "着边",
    "著重": "着重",
    "著陸": "着陆",
    "著鞭": "着鞭",
    "著題": "着题",
    "著驚": "着惊",
    "著體": "着体",
    "著魔": "着魔",
    "著麼": "着么",
    "葛摩": "科摩罗",
    "葛萊美獎": "格莱美奖",
    "蒙特內哥羅": "黑山",
    "蒙特婁": "蒙特利尔",
    "蒙著": "蒙着",
    "蒲朗克": "普朗克",
    "蒲美蓬": "普密蓬",
    "蒲隆地": "布隆迪",
    "蓋亞那": "圭亚那",
    "蓋著": "盖着",
    "蓋達組織": "基地组织",
    "蕭士塔高維奇": "肖斯塔科维奇",
    "蕭士達高維契": "肖斯塔科维奇",
    "薛丁格": "薛定谔",
    "薰心": "熏心",
    "薰沐": "熏沐",
    "薰習": "熏习",
    "薰陶": "熏陶",
    "薰香": "熏香",
    "藉著": "借着",
    "藍芽": "蓝牙",
    "藏著": "藏着",
    "蘇利南": "苏里南",
    "蘇辛尼津": "索尔仁尼琴",
    "蘊涵著": "蕴涵着",
    "蘭卡威": "浮罗交怡",
    "蘭茜·佩洛西": "南希·佩洛西",
    "蘸著": "蘸着",
    "蘿莉塔": "洛丽塔",
    "處境喜劇": "情景喜剧",
    "虛擬實境": "虚拟现实",
    "螢光棒": "荧光棒",
    "螢屏": "荧屏",
    "行人路": "人行道",
    "行動作業系統": "移动操作系统",
    "行動應用程式": "移动应用程序",
    "行動支付": "移动支付",
    "行動網路": "移动网络",
    "行動網際網路": "移动互联网",
    "行動裝置": "移动设备",
    "行動電話": "移动电话",
    "行政總裁": "首席执行官",
    "行著": "行着",
    "行餘": "行馀",
    "衝著": "冲着",
    "衣索匹亞": "埃塞俄比亚",
    "衣索比亞": "埃塞俄比亚",
    "衣著": "衣着",
    "表姊": "表姐",
    "裝著": "装着",
    "裴洛西": "佩洛西",
    "裹著": "裹着",
    "複製人": "克隆人",
    "西元1": "公元1",
    "西元2": "公元2",
    "西元3": "公元3",
    "西元4": "公元4",
    "西元5": "公元5",
    "西元6": "公元6",
    "西元7": "公元7",
    "西元8": "公元8",
    "西元9": "公元9",
    "西元前": "公元前",
    "西洋棋": "国际象棋",
    "西臺人": "赫梯人",
    "西臺國": "赫梯国",
    "西臺帝": "赫梯帝",
    "西臺文": "赫梯文",
    "西臺族": "赫梯族",
    "西臺王": "赫梯王",
    "西臺語": "赫梯语",
    "要帳": "要账",
    "覆審": "复审",
    "覆查": "复查",
    "覆核": "复核",
    "覆檢": "复检",
    "覆著": "覆着",
    "覆蓋著": "覆盖着",
    "規畫": "规划",
    "視著": "视着",
    "觔斗": "斤斗",
    "解像度": "分辨率",
    "解析度": "分辨率",
    "託庸": "讬庸",
    "託恩多": "讬恩多",
    "託麻": "讬麻",
    "記帳": "记账",
    "記著": "记着",
    "記錄著": "记录着",
    "試著": "试着",
    "認帳": "认账",
    "語著": "语着",
    "說著": "说着",
    "諠譁": "喧哗",
    "諾克少": "努瓦克肖特",
    "諾曼第": "诺曼底",
    "警戒著": "警戒着",
    "護著": "护着",
    "變著": "变着",
    "豎著": "竖着",
    "象徵著": "象征着",
    "象牙海岸": "科特迪瓦",
    "貝南共和國": "贝宁共和国",
    "貝爾格勒": "贝尔格莱德",
    "貝里斯": "伯利兹",
    "負著": "负着",
    "財務長，": "首席财务官，",
    "財務長、": "首席财务官、",
    "財務長。": "首席财务官。",
    "買帳": "买账",
    "資料庫": "数据库",
    "資訊時代": "信息时代",
    "資訊理論": "信息论",
    "賈伯斯": "乔布斯",
    "賒帳": "赊账",
    "賓·拉登": "本·拉登",
    "賓拉登": "本拉登",
    "賴帳": "赖账",
    "賴比瑞亞": "利比里亚",
    "賴索托": "莱索托",
    "賴著": "赖着",
    "賽普勒斯": "塞浦路斯",
    "赫魯雪夫": "赫鲁晓夫",
    "走為上著": "走为上着",
    "走著": "走着",
    "趁著": "趁着",
    "超連結": "超链接",
    "趕著": "赶着",
    "趴著": "趴着",
    "跑著": "跑着",
    "跟著": "跟着",
    "跪著": "跪着",
    "跳著": "跳着",
    "跼促": "局促",
    "跼限": "局限",
    "踏著": "踏着",
    "踩著": "踩着",
    "蹲著": "蹲着",
    "躍著": "跃着",
    "身分": "身份",
    "身著": "身着",
    "躺著": "躺着",
    "車諾比": "切尔诺贝利",
    "軟碟機": "软驱",
    "軟體": "软件",
    "載著": "载着",
    "轉帳": "转账",
    "轉殖": "克隆",
    "轉著": "转着",
    "辛巴威": "津巴布韦",
    "辛康納利": "肖恩·康纳利",
    "辦著": "办着",
    "迎著": "迎着",
    "迦納": "加纳",
    "迪特里克堡": "德特里克堡",
    "迫著": "迫着",
    "迴圈": "循环",
    "追著": "追着",
    "逆著": "逆着",
    "逕入": "径入",
    "逕到": "径到",
    "逕取": "径取",
    "逕啟": "径启",
    "逕寄": "径寄",
    "逕庭": "径庭",
    "逕往": "径往",
    "逕流": "径流",
    "逕稱": "径称",
    "逕自": "径自",
    "逕行": "径行",
    "逕迎": "径迎",
    "連著": "连着",
    "進帳": "进账",
    "逼著": "逼着",
    "遇著": "遇着",
    "運算元": "算子",
    "遍佈著": "遍布着",
    "遍布著": "遍布着",
    "過份": "过分",
    "過著": "过着",
    "道瓊": "道琼斯",
    "達·文西": "达·芬奇",
    "達文西": "达芬奇",
    "達著": "达着",
    "還帳": "还账",
    "邱吉爾": "丘吉尔",
    "部份": "部分",
    "都卜勒": "多普勒",
    "都等著": "都等着",
    "配合著": "配合着",
    "配著": "配着",
    "醉瀋": "醉渖",
    "醜著": "丑着",
    "釀著": "酿着",
    "金夏沙": "金沙萨",
    "金氏世界": "吉尼斯世界",
    "銷帳": "销账",
    "鋪著": "铺着",
    "錄影帶": "录像带",
    "鍊狀": "链状",
    "鍊表": "链表",
    "鏈結": "链接",
    "鐵達尼號": "泰坦尼克号",
    "閉著": "闭着",
    "開帳": "开账",
    "開著": "开着",
    "閑著": "闲着",
    "閒著": "闲着",
    "閤家": "阖家",
    "閻懷禮": "闫怀礼",
    "關係著": "关系着",
    "關帳": "关账",
    "關著": "关着",
    "阿士匹靈": "阿司匹林",
    "阿布達比": "阿布扎比",
    "阿拉伯聯合大公國": "阿拉伯联合酋长国",
    "阿斯匹靈": "阿司匹林",
    "阿爾蓋達組織": "基地组织",
    "阿迪斯阿貝巴": "亚的斯亚贝巴",
    "附著": "附着",
    "陋著": "陋着",
    "陪著": "陪着",
    "陰沈": "阴沉",
    "陳乾生": "陈乾生",
    "陳公乾生": "陈公乾生",
    "陳遇乾": "陈遇乾",
    "陸徵祥": "陆徵祥",
    "陽為乾": "阳为乾",
    "隔著": "隔着",
    "隨著": "随着",
    "雅爾達": "雅尔塔",
    "雅穆索戈": "亚穆苏克罗",
    "雅著": "雅着",
    "雜著": "杂着",
    "雜訊": "噪声",
    "雪鍊": "雪链",
    "雷伊泰灣": "莱特湾",
    "雷射": "激光",
    "雷諾瓦": "雷诺阿",
    "電單車": "摩托车",
    "電晶體": "晶体管",
    "電腦程式": "计算机程序",
    "霍爾斯坦": "荷尔斯泰因",
    "非份": "非分",
    "非同步": "异步",
    "靠著": "靠着",
    "韌體": "固件",
    "韓戰": "朝鲜战争",
    "音效卡": "声卡",
    "響著": "响着",
    "頂著": "顶着",
    "順著": "顺着",
    "頓內次克": "顿涅茨克",
    "領著": "领着",
    "頻寬": "带宽",
    "顯示卡": "显卡",
    "颳著": "刮着",
    "飃著": "飘着",
    "飄著": "飘着",
    "飛彈": "导弹",
    "飽託": "饱讬",
    "餘慶": "馀庆",
    "餘瀋": "馀渖",
    "馬利共和國": "马里共和国",
    "馬勒當拿": "马拉多纳",
    "馬哈地·穆罕默德": "马哈蒂尔·穆罕默德",
    "馬哈迪·穆罕默德": "马哈蒂尔·穆罕默德",
    "馬拉威": "马拉维",
    "馬拉度納": "马拉多纳",
    "馬斯垂克": "马斯特里赫特",
    "馬爾他": "马耳他",
    "馬爾地夫": "马尔代夫",
    "馬鞌": "马鞍",
    "駕著": "驾着",
    "駛著": "驶着",
    "騎著": "骑着",
    "騙著": "骗着",
    "马哈迪·莫哈末": "马哈蒂尔·穆罕默德",
    "高畫質": "高清",
    "高著": "高着",
    "鬥著": "斗着",
    "鬧著": "闹着",
    "鹼菜": "硷菜",
    "鹽份": "盐分",
    "麻薩諸塞": "马萨诸塞",
    "麼氏": "麽氏",
    "麼著": "么着",
    "黃潤乾": "黄润乾",
    "黃霄雲": "黄霄雲",
    "黎克特制": "里氏",
    "黏著": "黏着",
    "黛安娜": "戴安娜",
    "點著": "点着",
    "點陣圖": "位图",
}


def convert_zh(converter: OpenCC, word: str) -> str:
    """繁体转换成简体"""
    return ZH_WORDS[word] if word in ZH_WORDS else converter.convert(word)


def filter_words(converter: OpenCC, words: list[str]) -> list[str]:
    """繁体转为简体并过滤重复词语，最后按长度排序"""
    word_out = set()
    trans = str.maketrans(ZH_CHARS)
    for w in words:
        w1 = w.translate(trans)
        w2 = convert_zh(converter, w1)
        if w2.lower() in word_out:
            continue
        word_out.add(w2)

    word_list = sorted(word_out, key=lambda x: (len(x), x.lower()))

    return word_list


def format_word(
    word: str,
    pattern_postfix: str,
    pattern_en,
    pattern_has_cjk: str,
    pattern_cjk: str,
    pattern_cjk_more: str,
    pattern_cjk_extra: str,
    min_word_len: int = 2,
) -> tuple[int, str]:
    word = unicodedata.normalize("NFKC", word.strip())  # 替换兼容字符
    word = re.sub(pattern_postfix, "", word.strip())
    word = re.sub(r"[\s_]+", " ", word).strip()

    flag = 0
    if len(word) < min_word_len:
        return flag, word
    if word.endswith(("/翻译", "/衍生词", "/翻譯", "/衍生詞")) or word.startswith("不支援的"):
        return flag, word

    if re.findall(pattern_has_cjk, word):
        if re.match(pattern_cjk, word):
            flag = 1
        else:
            word = re.sub(r"[・‧•]", "·", word)
            word = re.sub(r"[─–－]", "-", word)
            word = re.sub(r"([^—])—([^—])", r"\1-\2", word)
            word = re.sub(r"[／]", "/", word)
            word = re.sub(r"[．]", ".", word)

            if re.match(pattern_cjk_more, word):
                word = word.strip("·-/")
                flag = 2
            elif re.match(pattern_cjk_extra, word, flags=re.ASCII):
                flag = 3

    else:
        # 纯英文
        if re.match(pattern_en, word) and re.findall(r"\w", word, flags=re.ASCII):
            flag = 4
    return flag, word


CATE_NAMES = ["main", "more", "extra"]
EN_PUNCTUATION = "!\"#$%&'()*+,./:;<=>?@[\\]^_`{|}~"
ZH_PUNCTUATION = "‘’“”…、。〈〉《》「」『』【】〔〕·！（），：；？～—"
# ZH_PUNCTUATION2 = "§×†‡※□▭〃〻﹏＿／－—─"
CJK_CHAR = "〇一-龥龦-鿿㐀-䶿𠀀-𪛟𪜀-𫜹𫝀-𫠝𫠠-𬺡𬺰-𮯠𰀀-𱍊𱍐-𲎯𮯰-𮹝豈-龎丽-𪘀"
MIN_WORD_LEN = 2


# 转化
def format_texts(data_dir: str | Path, save_dir: str | Path, lines: int) -> None:
    zh_word = []
    zh_word_extra = []
    zh_word_more = []
    en_word = []
    unk_word = []

    pattern_postfix = r"_\(([^)]+)\)$"
    pattern_en = rf"^[\w{EN_PUNCTUATION}\- ]+$"
    pattern_has_cjk = rf"[{CJK_CHAR}]+"
    pattern_cjk = rf"^[{CJK_CHAR}]+$"
    pattern_cjk_more = rf"^[{CJK_CHAR}\-·/]+$"
    pattern_cjk_extra = rf"^[{CJK_CHAR}{EN_PUNCTUATION}{ZH_PUNCTUATION}\-\w ]+$"

    files = sorted(Path(data_dir).glob("*.tsv"))
    logging.info(f"ℹ️ {data_dir} 文件共有 = {len(files)}")

    for file in files:
        logging.debug(f"🔖 读取 {file}")

        # with open(file, encoding="utf-8") as f:
        data = pd.read_csv(file, sep="\t", keep_default_na=False)["title"].tolist()
        for word in data:
            # word = word.split("\t")[0]
            extra = re.findall(pattern_postfix, word)  # 补充后缀分类词汇
            for w in word.split("/") + extra:
                flag, out = format_word(
                    w,
                    pattern_postfix,
                    pattern_en,
                    pattern_has_cjk,
                    pattern_cjk,
                    pattern_cjk_more,
                    pattern_cjk_extra,
                    MIN_WORD_LEN,
                )
                if len(out) == 1:
                    flag = 0

                match flag:
                    case 1:
                        zh_word.append(out)
                    case 2:
                        zh_word_more.append(out)
                    case 3:
                        zh_word_extra.append(out)
                    case 4:
                        en_word.append(out)
                    case _:
                        unk_word.append(out)

    logging.info("初始化 opencc")
    converter = OpenCC("t2s.json")

    Path(save_dir).mkdir(parents=True, exist_ok=True)
    save_data = [zh_word, zh_word_more, zh_word_extra]
    for words, keyword in zip(save_data, CATE_NAMES):
        save_path = Path(save_dir, keyword + ".txt")
        assert "恋" not in words
        filtered_words = filter_words(converter, words)
        assert "恋" not in filtered_words
        if keyword == CATE_NAMES[-1]:
            filtered_words = sorted(filtered_words)

        logging.info(f"ℹ️ {save_path} 过滤后词数 {len(words)} => {len(filtered_words)}")
        # split_words_by_lines(filtered_words, save_path, lines_per_file=lines)
        with open(save_path, "w", encoding="utf-8") as f:
            f.write("\n".join(filtered_words))


def stats_data(data_dir: str, out_dir: str):
    files = Path(data_dir).glob("*.json")
    save_file = Path(out_dir, "data.tsv")

    words = {}
    for file in files:
        with open(file, encoding="utf-8") as f:
            data = json.load(f)
            if "query" in data and "allpages" in data["query"]:
                for v in data["query"]["allpages"]:
                    t = v["title"]
                    if t not in words:
                        words[t] = 0
                    words[t] += 1
    logging.info(f"words = {len(words)}")
    df = pd.Series(words).reset_index()
    df.columns = ["title", "count"]
    df = df.sort_values(["title", "count"], ascending=[True, False])

    logging.info(f"Save words to {save_file}")
    if not Path(out_dir).exists():
        Path(out_dir).mkdir(parents=True)
    df.to_csv(save_file, index=False, sep="\t")


if __name__ == "__main__":
    fmt = "%(asctime)s %(filename)s [line:%(lineno)d] %(levelname)s %(message)s"
    logging.basicConfig(level=logging.INFO, format=fmt)

    stats_data("out", "raw")
    format_texts("raw", "dict", 1)
